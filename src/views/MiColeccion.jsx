...
function parseEuroToNumber(s) {
  ...
  if (raw === "") return null;
  // admitir coma decimal y espacios
  const normalized = raw.replace(/\./g, "").replace(",", "."); // “1.234,56” -> “1234.56”
...
function pickBestMatchBySetAndNumber(results, detail) {
  ...
  if (setName) {
    const bySet = results.find(
      (r) => (r.set_name || "").toLowerCase() === setName
    );
...
export default function MiColeccion() {
  ...
  useEffect(() => {
    ...
    const initialPrice =
      detail?.paid_eur != null
        ? Number(detail.paid_eur)
        : detail?.eur != null
        ? Number(detail.eur)
        : 0;
    setPriceInput(
    ...
    (async () => {
      try {
        ...
        let fetched = null;
        const idGuess =
          typeof detail?.scry_id === "string" && detail.scry_id.length === 36
            ? detail.scry_id
            : null;
        ...
        if (
          !cancelled &&
          (detail?.paid_eur == null || Number(detail.paid_eur) === 0)
        ) {
  ...
  async function handleSave() {
    try {
      ...
      const paidParsed = parseEuroToNumber(priceInput);
      // Permitimos null (sin dato), pero si no es null debe ser número válido
      if (priceInput !== "" && (isNaN(paidParsed) || paidParsed < 0)) {
        alert("Precio de compra inválido. Usa formato 1234.56 o 1.234,56");
        return;
      }

      const payload = {
        cardId,
        paid_eur: priceInput === "" ? null : paidParsed, // null o número válido
      ...
      if (res?.ok === false) {
        // Ver el motivo exacto que devuelve el backend
        console.error("[collectionUpdatePaid] error:", res);
        alert("Error al guardar: " + (res?.error || "desconocido"));
        return;
      }

      await loadData();
      // refrescamos el detalle local mostrado
            ...
            <div className="modal-body">
                  ...
                  <div className="info-cols">
                    <div className="kv">
                      <div>
                        ...
                        <span className="v">
                          {view?.setName || "—"}
                          {view?.setCode ? ` (${view.setCode})` : ""}
                        </span>
                      </div>
                      <div>
                        <span className="k">Número</span>{" "}
                        <span className="v">
                          {view?.collector || detail?.collector_number || "—"}
                        </span>
                      </div>
                      <div>
                        <span className="k">Rareza</span>{" "}
                        <span className="v">
                          {view?.rarity || detail?.rarity || "—"}
                        </span>
                      </div>
                      <div>
                        <span className="k">Coste</span>{" "}
                        <span className="v mono">{view?.manaCost || "—"}</span>
                      </div>
                      <div>
                        <span className="k">Tipo</span>{" "}
                        <span className="v">{view?.typeLine || "—"}</span>
                      </div>
                      {view?.power != null && view?.toughness != null && (
                        <div>
                          <span className="k">P/T</span>{" "}
                          <span className="v">
                      ...
                      {view?.loyalty != null && (
                        <div>
                          <span className="k">Lealtad</span>{" "}
                          <span className="v">{view.loyalty}</span>
                        </div>
                      )}
                      <div>
                      ...
                      <div>
                        <span className="k">Identidad</span>{" "}
                        <span className="v">
                          {(view?.colorIdentity || []).join(", ") || "—"}
                        </span>
                      </div>
                      <div>
                        <span className="k">Artista</span>{" "}
                        <span className="v">{view?.artist || "—"}</span>
                      </div>
                      <div className="precios">
                        <span className="k">Precio</span>
                        <span className="v">
                          {view?.priceEur != null
                            ? `EUR ${view.priceEur.toFixed(2)}`
                            : "—"}
                          {view?.priceUsd != null
                            ? ` · USD ${view.priceUsd.toFixed(2)}`
                            : ""}
                          {view?.priceTix != null
                            ? ` · TIX ${view.priceTix}`
                            : ""}
              ...
              <div className="form-row">
                ...
                <input
                  ...
                  type="text" // <- text para permitir “,” sin pelear con el control
                  inputMode="decimal"
                  placeholder="0,00"
                  value={priceInput}
                  onChange={(e) => setPriceInput(e.target.value)}
                  onBlur={(e) => {
                    // normaliza visualmente a “12,34” con coma si quieres:
