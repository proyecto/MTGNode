...
try {
  ...
  const api = {
    ...
    addToCollection: (cardId, qty = 1) => ipcRenderer.invoke('collection:add', { cardId, qty }),
    collectionUpdateQty: (cardId, qty) => ipcRenderer.invoke('collection:updateQty', { cardId, qty }),
    collectionRemove: (cardId) => ipcRenderer.invoke('collection:remove', { cardId }),
    // Scry
    ...
    scryAddToCollection: (payload) => ipcRenderer.invoke('scry:addToCollection', payload),
