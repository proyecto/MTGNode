...
function tableInfo(conn, table) {
  try { return conn.prepare(`PRAGMA table_info(${table})`).all(); } catch { return []; }
}
...
export const CollectionController = {
  diag() {
    ...
    if (has('scry_id')) {
      withScry = conn.prepare(`SELECT COUNT(*) AS n FROM collection WHERE scry_id IS NOT NULL AND scry_id <> ''`).get().n;
  ...
  addFromScry(payload, qty = 1) {
    ...
    if (hasColumn(conn, 'collection', 'scry_id') && payload?.id) {
      cols.push('scry_id'); qms.push('?'); vals.push(payload.id);
    }
    if (hasColumn(conn, 'collection', 'name') && payload?.name) {
      cols.push('name'); qms.push('?'); vals.push(payload.name);
    }
    if (hasColumn(conn, 'collection', 'set_name') && payload?.set_name) {
      cols.push('set_name'); qms.push('?'); vals.push(payload.set_name);
    }
    if (hasColumn(conn, 'collection', 'rarity') && payload?.rarity) {
      cols.push('rarity'); qms.push('?'); vals.push(payload.rarity);
    }
    ...
    try {
      ...
      if (String(err?.code) === 'SQLITE_CONSTRAINT_UNIQUE' && hasColumn(conn, 'collection', 'card_id') && cardId != null) {
